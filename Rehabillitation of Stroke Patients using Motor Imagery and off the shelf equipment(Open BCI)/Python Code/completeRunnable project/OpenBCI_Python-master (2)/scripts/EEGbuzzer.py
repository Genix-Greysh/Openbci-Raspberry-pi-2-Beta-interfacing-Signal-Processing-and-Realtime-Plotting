...
def dataHandlingFromOpenbci(sample,bcint):
	...
	if( (sample.id % 251) == 0 and sample.id != 0):
		timeInSeconds = timeInSeconds+1;
                lap=[];
                mid=[];
                storeChannel2dat = signal.detrend(storeChannel2data);
                storeChannel4dat = signal.detrend(storeChannel4data);
                storeChannel6dat = signal.detrend(storeChannel6data);
                mid = [2*r for r in storeChannel4dat]
                for i in range(len(storeChannel4dat)):
                        lap.append(mid[i]-storeChannel2dat[i]-storeChannel6dat[i])
		onesec = [];
		lowcut = 16;
        	highcut = 24;
		tmpstoreChanneldata = butter_bandpass_filter(lap, lowcut, highcut, fs, order=6)  
                ...
                for i in range(len(tmpstoreChanneldata)):
                        normalized.append(tmpstoreChanneldata[i]/ 1000 );
                onesecnormalized = np.array(normalized[-250:])
                
                bzr = (1.0/250.0)*sum(abs(onesecnormalized));
                print bzr;
                #if(bzr > 0.01 and (len(normalized) >= 500)) :
                if(bzr > 0.01):
